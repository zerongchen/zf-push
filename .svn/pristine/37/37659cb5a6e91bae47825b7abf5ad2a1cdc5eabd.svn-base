...
/**
 * A TextProcessor is a convenient class for handling some common text processing.
 * @author Jason.CW.Cheung
 * @date 2017年7月26日 下午5:19:54
 * @version 1.0
 */
public class StringUtil {
	...
	public static boolean isWebSite(String s) {
		...
		Pattern pattern = Pattern.compile("^[hH]ttp://[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+\\.?$");
		Matcher matcher = pattern.matcher(s);
		if (matcher.matches()) {
			return true;
		} else {
	...
	public static boolean isUrl(String s) {
		...
		Pattern pattern = Pattern.compile("^((http|Http|https|Https|ftp|Ftp|file|File)://){0,1}[a-zA-Z0-9][-/_\\|a-zA-Z0-9]{0,62}(\\.[a-zA-Z0-9][-/_\\|a-zA-Z0-9]{0,62})+\\.?$");
		Matcher matcher = pattern.matcher(s);
		if (matcher.matches()) {
			return true;
		} else {
	...
	public static boolean isOctalString(String s) {
		if(isEmptyString(s)){
			return false;
		}
